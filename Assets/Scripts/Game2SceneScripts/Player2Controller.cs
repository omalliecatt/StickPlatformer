...
public class Player2Controller : MonoBehaviour
{
    ...
    void movementController()
    {
        ...
        if (Input.GetKeyDown(KeyCode.UpArrow) && move2 == 0 && animState != 3 && !isGameOver)
        {
            jumpSource.GetComponent<AudioSource>().Play();
            jump = 1;
        }
        else if (Input.GetKey(KeyCode.RightArrow) && animState != 3 && !isGameOver)
        {
            if (move < maxSpeed)
                move += 1f;
        }
        else if (Input.GetKey(KeyCode.LeftArrow) && animState != 3 && !isGameOver)
        {
            if (move > (-maxSpeed))
                move -= 1f;
        }
